...
exports.getCrewsByMember =  function(personId, extraQuery,  callback){
...
module.exports = function(db) {
    ...
    crew.getAllCrews = function( callback){
        ...
        db.getConnection(function(err, connection){
            ...
            var query = ' SELECT crew_id, c.owner_id, p.first_name owner, c.name, ( SELECT count(*) FROM crew_member WHERE crew_id = c.crew_id ) member_count ' +
    ...
    crew.addNewCrew = function(personId, name, callback){
        db.getConnection(function(err, connection){
            connection.query( ' INSERT INTO crew (owner_id, name) VALUES (?, ?)', [personId, name],
                function(err, result) {
                    if (err) {
                        callback({me: err});
                    }
